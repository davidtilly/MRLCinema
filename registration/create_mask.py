...
def mask_dilation(mask:np.array, pos_000:np.array, spacing:np.array, dilation_distance) -> np.array:
    """
    Create a SimpleITK image from a numpy array. The image is created with the
    same origin and spacing as the original image.
    ...
    dt = distance_map(mask, spacing)
    dt = dt + dilation_distance 
    mask_dilated = (dt >= 0).astype(np.uint8)

    return mask_dilated


def create_mask_2d(mask:np.array, pos_000:np.array, spacing:np.array, slice_diretion:SliceDirection) -> sitk.Image:
    """
...
def remove_center_cross(mask:np.array, pos_00:np.array, spacing:np.array, block_size:int) -> np.array:

    dim = mask.shape
    center = [int(dim[0]/2), int(dim[1]/2)]
    mask_remove = mask.copy()
    mask_remove[center[0]-block_size:center[0]+block_size, center[1]-block_size:center[1]+block_size] = 0
    return mask_remove
